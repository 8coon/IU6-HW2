...
public class SyntaxAnalyzer {
    ...
    private ASTItem scanExpression(int start) throws SyntaxException {
        if (this.tokens.size() <= start + 2) {
            throw new SyntaxException(this.tokens.get(start).getPosition(), "Unfinished statement");
        }

        if (!this.tokens.get(start).getType().equals(LexicalState.IDENTIFIER)) {
            throw new SyntaxException(this.tokens.get(start).getPosition(), "Expected identifier");
        }

        if (!(this.tokens.get(start + 1).getType().equals(LexicalState.SYMBOL) &&
                this.tokens.get(start + 1).getRead().equals("="))) {
            throw new SyntaxException(this.tokens.get(start + 1).getPosition(), "Expected =");
        }
        ...
        if (!(
            ...
            throw new SyntaxException(this.tokens.get(start + 2).getPosition(), "Expected identifier or constant");
        }
    ...
    private ASTItem scanIf(int start) throws SyntaxException {
        if (!tokens.get(start).getType().equals(LexicalState.IF)) {
            throw new SyntaxException(tokens.get(start).getPosition(), "Expected IF");
        }

        if (!LexicalState.SYMBOL.equals(tokens.get(start + 1).getType()) &&
                "(".equals(tokens.get(start + 1).getRead())) {
            throw new SyntaxException(tokens.get(start + 1).getPosition(), "Expected (");
        }

        if (!LexicalState.IDENTIFIER.equals(tokens.get(start + 2).getType())) {
            throw new SyntaxException(tokens.get(start + 2).getPosition(), "Expected identifier");
        }

        if (!(LexicalState.SYMBOL.equals(tokens.get(start + 3).getType()) &&
                ")".equals(tokens.get(start + 3).getRead()))) {
            throw new SyntaxException(tokens.get(start + 3).getPosition(), "Expected )");
        }
        ...
        if (!tokens.get(condition.newPosition).getType().equals(LexicalState.ELSE)) {
            throw new SyntaxException(tokens.get(condition.newPosition).getPosition(), "Expected ELSE");
        }
